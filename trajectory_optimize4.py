...
class LMConfig:
    ...
    # Hard projections to avoid IK-branch flips / discontinuities
    max_seg_delta_deg: float = 4.0   # hard clamp |q[i+1]-q[i]| per joint
    seed_trust_deg: float = 25.0     # hard clamp |q[i]-q_seed[i]| per joint
    use_seed_init: bool = True       # initialize with stable sequential IK seed
...
def _clamp_segment_deltas(Q: np.ndarray, max_delta: float) -> np.ndarray:
    """Hard clamp per-joint step between consecutive waypoints, preserving endpoints."""
    Q = Q.copy()
...
def _clamp_to_seed(Q: np.ndarray, Q_seed: np.ndarray, trust: float) -> np.ndarray:
    """Hard clamp each waypoint to remain within +/- trust degrees of the seed."""
    trust = float(trust)
    Q = Q.copy()
    lo = Q_seed - trust
    hi = Q_seed + trust
...
def _stable_sequential_seed(
    ...
    max_waypoint_delta_deg: float = 3.0,
    iters_per_point: int = 60,
) -> np.ndarray:
    """
    Build a consistent IK-branch seed by solving waypoints sequentially with
    a trust region to the previous waypoint (prevents 180° flips).
    """
    N = P_target.shape[0]
    D = len(joints)
    Q = np.zeros((N, D), dtype=float)

    # Start from interpolation
    for i in range(N):
        s = i / (N - 1)
        Q[i] = (1 - s) * q0 + s * qT
    Q[0], Q[-1] = q0, qT
    ...
    for i in range(1, N - 1):
        ...
        for _ in range(iters_per_point):
            ...
            e = P_target[i] - p
            if float(np.linalg.norm(e)) < 0.8:
                break

            J = numerical_jacobian_pos(arm, q, joints, eps_deg=jac_eps_deg)
            ...
            # trust region to previous waypoint
            dq_wp = q_try - Q[i - 1]
            dq_wp = np.clip(dq_wp, -max_waypoint_delta_deg, +max_waypoint_delta_deg)
            q_try = Q[i - 1] + dq_wp
            q_try = _project_to_limits(arm, q_try, joints)

            # accept if improves, else increase damping
            if float(np.linalg.norm(P_target[i] - _fk_end(arm, q_try, joints))) <= float(np.linalg.norm(e)) + 1e-9:
                q = q_try
                lam = max(1.0, lam * 0.95)
            else:
                lam = min(200.0, lam * 1.2)
...
def solve_batch_lm(
    ...
    # Build a consistent seed to keep a single IK branch (optional)
    Q_seed = None
    if cfg.use_seed_init:
        Q_seed = _stable_sequential_seed(
            arm=arm,
            q0=q0,
            qT=qT,
            P_target=P_target,
            joints=joints,
            jac_eps_deg=cfg.jac_eps_deg,
            damping=12.0,
            max_step_deg=2.0,
            max_waypoint_delta_deg=min(3.0, cfg.max_seg_delta_deg),
            iters_per_point=60,
        )
    ...
    # Hard projections on the initial guess
    Q = _clamp_segment_deltas(Q, cfg.max_seg_delta_deg)
    if Q_seed is not None:
        Q = _clamp_to_seed(Q, Q_seed, cfg.seed_trust_deg)
    x = _pack(Q)
    ...
    def loss_from_Q(Qcur: np.ndarray) -> float:
        Qt = Qcur.copy()
        Qt = _clamp_segment_deltas(Qt, cfg.max_seg_delta_deg)
        if Q_seed is not None:
            Qt = _clamp_to_seed(Qt, Q_seed, cfg.seed_trust_deg)
        for ii in range(1, N - 1):
            Qt[ii] = _project_to_limits(arm, Qt[ii], joints)
        r, _ = build_residual_and_jacobian(arm, Qt, P_target, cfg, q_mid, q_rng)
        return float(r @ r)
    ...
    for _it in range(cfg.iters):
        ...
        for _ls in range(cfg.line_search_steps):
            ...
            Q_try = _clamp_segment_deltas(Q_try, cfg.max_seg_delta_deg)
            if Q_seed is not None:
                Q_try = _clamp_to_seed(Q_try, Q_seed, cfg.seed_trust_deg)
            for i in range(1, N - 1):
                Q_try[i] = _project_to_limits(arm, Q_try[i], joints)
...
if __name__ == "__main__":
    ...
    plot_dashboard(res, title="Batch LM branch-safe")
