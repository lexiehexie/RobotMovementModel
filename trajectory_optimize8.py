...
def dijkstra_cartesian_shortest(
    arm: fkmodel.Manipulator6DOF,
    q_start: Dict[int, float],
    q_goal: Dict[int, float],
    cfg: DijkstraCartesianConfig,
) -> Dict[str, Any]:
    joints = cfg.joints
    D = len(joints)
    specs = arm.servo_specs()
    ...
    while open_heap:
        ...
        if expansions >= cfg.max_expansions:
            print(f"[Dijkstra-Cart] max_expansions reached; using best_goal_dist={best_goal_dist:.3f}")
            break

        rss_mb = _rss_mb()
        if rss_mb == rss_mb and rss_mb >= float(cfg.max_memory_mb):
            print(f"[Dijkstra-Cart] max_memory_mb reached ({rss_mb:.1f} MB); using best_goal_dist={best_goal_dist:.3f}")
            break
        ...
        if cfg.progress_every and expansions % cfg.progress_every == 0:
            ...
            print(f"[Dijkstra-Cart] expanded={expansions:,} open={open_size:,} touched={len(key_to_q):,} "
        ...
        for d in range(D):
            for sgn in (-1.0, +1.0):
                ...
                pn = fk_cached(qn)
                edge_cost = float(np.linalg.norm(pn - pcur))
    ...
    while path_keys[-1] != start_key:
        ...
        if parent is None:
            print(f"[Dijkstra-Cart] Warning: broken parent chain at {cur}; returning partial path.")
            break
...
if __name__ == "__main__":
    ...
    res = dijkstra_cartesian_shortest(arm, q_start, q_end, cfg)

    print("Dijkstra-Cart expansions:", res["expansions"])
    ...
    try:
        ...
        plot_dashboard(res, title=f"Dijkstra Cartesian shortest (step={cfg.step_deg}°)")
